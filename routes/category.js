...
// 商品分类删除
router.get("/deleteTopCategory", checkRootLogin)
router.get("/deleteTopCategory", function (req, res) {
    console.log(req.body.id);
    var categoryId = new Category({
        id: req.body.id ? req.body.id : '',
    })
    Category.deleteTopCategory(categoryId, function (err, data) {
        console.log(err);
        if (err) return res.send({ "error": 403, "message": "数据库异常！" });
        res.send({ "success": true });
    })
...
router.post("/addSecondCategory", function (req, res) {
    ...
    Category.addSecondCategory(brand, function (err, data) {
        if (err) return res.send({ "error": 403, "message": "数据库异常！" });
        res.send({ "success": true });
